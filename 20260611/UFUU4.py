...
def build_tree(depth: int, fold_func, fold_name: str):
    ...
    level_values = [leaves.copy()]  # for entropy profile
...
# ====================== MEASUREMENTS ======================
def compute_correlation(leaves, depth, n_samples=50000):
    n = len(leaves)
...
def compute_entropy_profile(level_values):
    ...
    for vals in level_values:
        # Discretize to 100 bins for Shannon entropy
...
def simple_attractor_scan(fold_func, fold_name, n_samples=1000):
    fixed = set()
    cycles = []
    for _ in range(n_samples):
        x = np.random.uniform(0, 1)
        seen = {}
        for step in range(20):  # short orbit search
            if fold_name == "XOR-Carry":
                nxt = fold_func(x, x)[0]
            elif fold_name == "pACK-G2":
                nxt = fold_pack_g2(x, x, 0, 10)  # dummy lca/d
            else:
                nxt = fold_func(x, x)
            if nxt in seen:
                period = step - seen[nxt]
                if period == 0:
                    fixed.add(round(nxt, 6))
                elif period <= 6:
                    cycles.append((round(nxt, 6), period))
                break
            seen[nxt] = step
            x = nxt
...
# ====================== MAIN FULL TEST ======================
def full_test(depth=12, plot=True):
    print(f"=== FULL RECURSIVE FOLD TEST — depth d={depth} ===\n")
    ...
    test_folds = {
        ...
        "Möbius": fold_mobius,
        "pACK-G2 (gap-closing)": None
    }
    
    for name, func in test_folds.items():
        print(f"→ {name}")
        leaves, root, level_vals = build_tree(depth, func if func else None, name)
        
        r_bins, C_r, slope, r2 = compute_correlation(leaves, depth)
        ent_profile = compute_entropy_profile(level_vals)
        n_fixed, n_cycles = simple_attractor_scan(func if func else fold_pack_g2, name)
        
        print(f"   Root value          : {root:.6f}")
        print(f"   Power-law exponent  : {slope:.3f} (R² = {r2:.3f})")
        print(f"   Entropy profile (root level) : {ent_profile[-1]:.3f}")
        print(f"   Attractors          : {n_fixed} fixed points, {n_cycles} short cycles")
        print(f"   Mean leaf           : {leaves.mean():.4f}")
        print("-" * 80)
        
        if plot and name in ["Golden", "Möbius", "pACK-G2 (gap-closing)"]:
...
if __name__ == "__main__":
    full_test(depth=12, plot=True)   # ← change to 14 when ready