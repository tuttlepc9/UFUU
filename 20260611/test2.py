...
def fold_xor_carry(a: float, b: float) -> tuple:
    """Returns (XOR, AND). We use XOR channel for scalar tests."""
    # Safe integer conversion for bitwise ops
    ia, ib = int(a), int(b)
    return (ia ^ ib, ia & ib)
    ...
    for level in range(1, depth + 1):
        ...
        for i in range(parent_size):
            ...
            if fold_name == "XOR-Carry":
                # Take XOR channel only (as per paper's multi-channel note)
...
def compute_correlation(leaves: np.ndarray, depth: int, n_samples=50000):
    n = len(leaves)
    coords = np.array([(i & ((1<<depth)-1), i >> depth) for i in range(n)])  # simple x-y from bits
    ...
    log_C = np.log(C_r[valid] + 1e-12)  # tiny offset to avoid log(0)
...
def run_test(depth=12, plot=False):
    ...
    for name, func in test_folds.items():
        ...
        if plot and name in ["Golden", "Möbius", "pACK-G2 (exploratory)"]:
            ...
            plt.ylabel('C(r)')
            plt.title(f'Spatial correlation — {name}')
            plt.legend()
            plt.grid(True, which='both', alpha=0.3)
            plt.show()

if __name__ == "__main__":
    run_test(depth=12, plot=True)   # set plot=False if you don't want figures