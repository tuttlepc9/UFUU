    ...
    for _ in range(n_samples):
        ...
        for step in range(20):
            if fold_name == "XOR-Carry":
                ...
                nxt = fold_pack_g2(x, x, 0, 10)  # dummy
            else:
                nxt = fold_func(x, x)
            if nxt in seen:
                period = step - seen[nxt]
                if period == 0:
                    fixed.add(round(float(nxt), 6))
                elif 1 <= period <= 6:
                    cycles.append((round(float(nxt), 6), period))
                break
            seen[nxt] = step
            x = nxt
...
def full_test(depth=12, plot=True):
    ...
    test_folds = {
        ...
        "Möbius": fold_mobius,
        "pACK-G2 (gap-closing)": None   # special-cased inside build_tree
    }
