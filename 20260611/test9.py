...
# ====================== pACK-G2 — TEST9 (MAX AGGRESSIVE) ======================
def fold_pack_g2(a, b, lca, d):
    """pACK-G2 test9 — strongest tuning for exponent ~−0.65 to −0.72 + attractors"""
    z = complex(a, b)
    conformal = np.abs((PHI * z + 1) / (z + PHI))
    exp_term = np.exp(-0.006 * (lca / d)**2)               # even slower large-scale decay
    carry_term = b / PHI
    diff_term = 0.68 * abs(a - b) * (2 ** -lca)            # MAX differentiation
    # Strong nonlinear Ricci-like curvature
    ricci_proxy = (0.35 * np.sin(np.pi * (a + b)) +
                   0.22 * (a - b)**3 +
                   0.12 * (a - b)**4 +
                   0.05 * (a - b)**5)
    # Extra lca-dependent decorrelation term (steepens slope at distance)
    decor_term = 0.09 * np.sin(2 * np.pi * lca / d) * (lca / d)**1.5
    return conformal * exp_term + carry_term + diff_term * (1 + ricci_proxy) + decor_term

# ====================== OTHER FOLDS (unchanged) ======================
...
def full_test(depth=14, plot=True):
    print(f"=== pACK-G2 TEST9 — MAX AGGRESSIVE TUNING — depth d={depth} ===\n")
...
if __name__ == "__main__":
    full_test(depth=14, plot=True)   # Try depth=16 for even richer structure