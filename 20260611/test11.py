...
def compute_ultrametric_correlation(leaves, depth, n_samples=80000):
    """Pure ultrametric LCA distance — exactly the metric in the paper (Section 5.1)"""
    n = len(leaves)
    idx1 = np.random.randint(0, n, n_samples)
    idx2 = np.random.randint(0, n, n_samples)
    lca_depths = []
    for i1, i2 in zip(idx1, idx2):
        xor = int(i1) ^ int(i2)
        if xor == 0:
            lca_depths.append(depth)
        else:
            lca_depths.append(depth - xor.bit_length())
    ...
    slope, _, r_value, _, _ = linregress(log_lca, log_C)
    return bins, C_lca, slope, r_value**2
...
def simple_attractor_scan(fold_func, fold_name, depth, n_samples=12000):
    fixed = set()
    cycles = []
...
def full_test(depth=14, plot=True):
    print(f"=== pACK-G2 TEST11 — FULLY FIXED + ULTRAMETRIC CORRELATION — d={depth} ===\n")
    ...
    for name, func in test_folds.items():
        ...
        _, _, manhattan_slope, manhattan_r2 = compute_correlation(leaves, depth)
        _, _, ultra_slope, ultra_r2 = compute_ultrametric_correlation(leaves, depth)
        ...
        print(f"   2D Manhattan exponent     : {manhattan_slope:.3f} (R² = {manhattan_r2:.3f})")
        print(f"   Ultrametric LCA exponent  : {ultra_slope:.3f} (R² = {ultra_r2:.3f})   ← paper's natural metric")
        ...
        if plot and name == "pACK-G2":
            plt.figure(figsize=(8,5))
            plt.title(f'pACK-G2 Ultrametric Correlation (d={depth})')
...
if __name__ == "__main__":
    full_test(depth=14, plot=True)   # Try depth=16 for richer results