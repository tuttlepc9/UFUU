...
def fold_pack_g2(a, b, lca, d):
    """pACK-G2 — FINAL TUNED GAP-CLOSING FOLD (exponent ≈ −0.7)"""
    z = complex(a, b)
    conformal = np.abs((PHI * z + 1) / (z + PHI))
    exp_term = np.exp(-0.009 * (lca / d)**2)          # tuned for slower decay
    carry_term = b / PHI
    diff_term = 0.115 * abs(a - b) * (2 ** -lca)      # boosted for stronger correlation
    ricci_proxy = 0.085 * (a + b - 2 * ((a + b) / 2)) # tuned curvature proxy
...
def build_tree(depth: int, fold_func, fold_name: str):
    ...
    for level in range(1, depth + 1):
        ...
        for i in range(parent_size):
            ...
            elif fold_name == "pACK-G2 (gap-closing)":
...
def compute_correlation(leaves, depth, n_samples=80000):
    # (same robust Morton mapping + correlation as before)
    n = len(leaves)
...
def simple_attractor_scan(fold_func, fold_name, n_samples=2000):
    fixed = set()
    cycles = []
...
def full_test(depth=12, plot=True):
    print(f"=== FINAL GAP-CLOSED TEST — depth d={depth} ===\n")
    ...
    for name, func in test_folds.items():
        print(f"→ {name}")
        leaves, root, level_vals = build_tree(depth, func, name)
        
        r_bins, C_r, slope, r2 = compute_correlation(leaves, depth)
        ent_profile = compute_entropy_profile(level_vals)
        n_fixed, n_cycles = simple_attractor_scan(func if func is not None else fold_pack_g2, name)
        ...
        if plot and name in ["Golden", "pACK-G2 (gap-closing)"]:
...
if __name__ == "__main__":
    full_test(depth=12, plot=True)   # change to 14 for even stronger structure