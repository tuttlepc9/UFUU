...
# ====================== STRONGLY TUNED pACK-G2 ======================
def fold_pack_g2(a, b, lca, d):
    """pACK-G2 FINAL TUNED — stronger diff + nonlinear curvature for ~−0.7 exponent + attractors"""
    z = complex(a, b)
    conformal = np.abs((PHI * z + 1) / (z + PHI))
    exp_term = np.exp(-0.011 * (lca / d)**2)           # slower large-scale decay
    carry_term = b / PHI
    diff_term = 0.245 * abs(a - b) * (2 ** -lca)       # STRONGER differentiation (as requested)
    # Nonlinear Ricci-like curvature proxy (sine + quadratic for richer dynamics)
    ricci_proxy = 0.14 * np.sin(np.pi * (a + b)) + 0.09 * (a - b)**2
...
def simple_attractor_scan(fold_func, fold_name, depth, n_samples=5000):
    fixed = set()
    cycles = []
...
def full_test(depth=14, plot=True):
    print(f"=== pACK-G2 FINAL TUNED TEST — depth d={depth} (target exponent ~−0.7 + attractors) ===\n")
...
if __name__ == "__main__":
    full_test(depth=14, plot=True)   # change to 16 if you want even stronger structure