...
Möbius (conformal) fold + GR-emergence graphics

Directly modeled on UFUUGR1.py for apples-to-apples comparison.
Implements the exact Möbius fold recommended in the paper (Sec 4.4).
Tests conformal/Lorentz emergence and the same GR curvature proxy.
"""
...
def binary_fraction(i: int, d: int) -> float:
    """Path-dependent leaf value c(path) ∈ [0,1) – same as golden-ratio script"""
    frac = 0.0
    for k in range(d):
        if (i & (1 << k)):
            frac += 0.5 ** (k + 1)
...
def mobius_fold(a: float, b: float) -> float:
    """Conformal Möbius fold Fm(a,b) = |(αz + β)/(γz + δ)| with paper-recommended params
    α = φ, β = 1, γ = 1, δ = φ  (SL(2,ℂ) det = φ² – 1 = φ)
    z = a + bi  → treats left/right siblings as real/imaginary parts
    """
    z = complex(a, b)
    num = PHI * z + 1
    den = z + PHI
...
def build_parent_values(d: int):
    """Builds the immediate parent level above the leaves (exactly as in UFUUGR1.py)"""
    num_parents = 1 << (d - 1)
...
def map_to_2d_grid(values: np.ndarray, d: int):
    """Interleaved-bit 2D mapping – identical to golden-ratio script"""
...
def compute_spatial_correlation(grid: np.ndarray, max_r: int = 30):
    """Two-point spatial correlation on Manhattan distance – same as UFUUGR1"""
    h, w = grid.shape
    corr = []
    dist = []
    mean_val = grid.mean()
...
def compute_graph_curvature(deltas: np.ndarray, alpha: float = ALPHA):
    """Same GR proxy as golden-ratio script (tests R = 8πT coupling)"""
...
if __name__ == "__main__":
    d = 12
    print(f"Running Möbius conformal fold at depth d = {d}")

    parents, deltas = build_parent_values(d)
    print(f"Last parent value (root-level proxy) ≈ {parents[-1]:.6f}")
    ...
    print(f"Graph Einstein residual (mean |R_proxy – 8πT_proxy|) ≈ {residual:.6f}")

    # ==================== GRAPHICS ====================
    fig, axs = plt.subplots(2, 2, figsize=(12, 10))

    # Top-left: Summary
    axs[0,0].text(0.5, 0.5, f'Möbius Conformal Fold\nDepth d = {d}\nLast parent ≈ {parents[-1]:.4f}\nSL(2,ℂ) symmetry test', 
    ...
    # Bottom-left: Honest GR panel (tests gravity emergence)
    ...
    plt.tight_layout()
    plt.savefig('mobius_fold_graphics.png', dpi=300, bbox_inches='tight')
    plt.show()

    print("\n✅ Graphics saved to 'mobius_fold_graphics.png'")
    print("   Möbius fold tested with identical diagnostics as golden-ratio fold.")
    print("   Compare spatial order (top-right) and GR residual (bottom-left) to UFUUGR1.")
    print("   Ready for deeper runs (d=14–16) or full-tree extension if residual is high.")