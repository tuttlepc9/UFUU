"""
UFUUMOB_RANDOM_BASELINE.py
Random Fold Baseline (M6/B2) vs Pure Möbius Fold
Identical 2D-grid GR test + spike ratio + P4 entropy
Seed fixed at 42 for reproducible comparison
"""
...
def fold_random(a: float, b: float) -> float:
    """Pure random fold — no relationship to inputs (baseline)"""
...
def build_tree(depth: int, fold_func):
    ...
    return leaves, current[0], level_values[-2]  # parents
...
# ====================== MAIN MULTI-DEPTH COMPARISON ======================
if __name__ == "__main__":
    depths = [12, 14, 15, 16, 18, 20, 22, 24]
    print("=== RANDOM FOLD BASELINE vs PURE MÖBIUS FOLD ===\n")
    print("d     | Fold       | Root       | Ultra slope | Peak LCA | Ratio   | 2D GR residual | P4 monotonic?")
    print("-" * 100)

    for d in depths:
        for fold_name, fold_func in [("Möbius", fold_mobius), ("Random", fold_random)]:
            ...
            print(f"{d:2d}    | {fold_name:6s}   | {root:.6f} | {ultra_slope:9.4f}  | "
                  f"{peak_lca:2d}      | {ratio:.3f}   | {residual:12.6f}   | {monotonic}")

    print("\n✅ Baseline comparison complete.")
    print("   If the Random fold residual stays flat/high while Möbius drops → fold is doing real work.")
    print("   If both residuals decay similarly → result is a grid artifact (not physics).")