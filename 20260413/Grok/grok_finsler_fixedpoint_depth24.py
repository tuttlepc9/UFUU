...
def finsler_quantum_fold(rho_L, rho_R):
    ...
    U2 = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]], dtype=complex)
...
print("=== Deep Fixed-Point Verifier — Finsler-Friedmann 2025 (Depth 24) ===\n")
U = random_mixed_state()
for step in range(25):
    U_new = finsler_quantum_fold(U, U)
    ...
    if diff < 1e-6:
        print("\n✓ Fixed point reached at depth 24.")
        break
    U = U_new
print("\nFinsler-Friedmann depth-24 verification complete.")