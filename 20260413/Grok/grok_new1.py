...
def fundamental_fold(state_L, state_R):
    """New fundamental fold built backwards from the observed universe.
    3 channels:
      0: acceleration / GR / cosmology
      1: entropy drive / variance preservation (P4)
      2: attractor selector / discrete scales (P5 + P3)
    """
    # Acceleration / GR proxy
    accel = 0.002 * (state_L[:,0]**2 + state_R[:,0]**2) * (state_L[:,0] - state_R[:,0])

    # Strong entropy drive that grows with depth (P4)
    depth_factor = 0.0045 * (DEPTH - np.log2(len(state_L) + 1e-8))
    entropy_drive = 0.007 * np.log1p(np.abs(state_L[:,1] * state_R[:,1]) + 1e-6) + \
                    0.005 * np.abs(state_L[:,1] - state_R[:,1]) + depth_factor

    # Attractor selector with strong diversity forcing (P5)
    diff = state_L[:,2] - state_R[:,2]
    attractor = 0.005 * (state_L[:,2]**3 - state_R[:,2]**3) + \
                0.0025 * np.sin(31.4 * (state_L[:,2] + state_R[:,2])) + \
                0.0018 * np.sign(diff) * np.abs(diff)**1.5
    ...
    # Low damping to keep the system alive
    return np.tanh(np.clip(new_state, -25, 25)) * 0.58

def leaves_vector(d):
    n = 1 << d
    base = np.arange(n, dtype=np.float64) % 17
    return np.stack([base, base * 0.7, base * 1.3], axis=1)   # 3-component seed

print("=== New Fundamental Fold — Built Backwards from the Final Answer (Depth 24) ===\n")
...
for level in range(DEPTH):
    left = g_vals[0::2]
    right = g_vals[1::2]
    g_vals = fundamental_fold(left, right)
...
for step in range(12):
    U_new = fundamental_fold(U.reshape(1,3), U.reshape(1,3))[0]
...
print("\n=== New Fundamental Fold complete ===")
print("This version was built by starting from the final observed universe and working backwards.")