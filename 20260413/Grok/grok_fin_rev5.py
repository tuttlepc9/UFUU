...
def ff_fold_v5(l, r):
    # Core Finsler acceleration (kept from what worked)
    accel = 0.0028 * (l**2 + r**2) * (l - r)
    
    # Strong symmetry breaking for multiple attractors (P5)
    nonlinear = 0.002 * (l**3 - r**3) + 0.0005 * (l**4 - r**4)
    
    # Periodic term for discrete scale structure (P3)
    periodic = 0.0015 * np.sin(12.56 * (l + r))   # higher frequency
    
    # Explicit entropy-driving term (P4)
    entropy_drive = 0.0012 * np.log1p(np.abs(l * r) + 1e-6)
    
    # Repulsive term to prevent single-value collapse
    repulsion = 0.0006 * (l - r)**2
    
    val = l + r + accel + nonlinear + periodic + entropy_drive + repulsion
    return safe_tanh(val) * 0.96
...
print("=== Finsler-Friedmann 2025 — TOE Checklist v5 (Depth 24) ===\n")
...
print("\n=== Finsler-Friedmann TOE Checklist v5 complete ===")
print("Paste the full output back. This v5 is the most aggressive redesign yet.")