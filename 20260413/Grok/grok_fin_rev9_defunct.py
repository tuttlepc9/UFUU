...
def ff_fold_v9(l, r):
    ...
    # Piecewise thresholds to force multiple distinct attractors (P5)
    diff = l - r
    nonlinear = 0.0
    if np.abs(diff) < 0.5:
        nonlinear = 0.004 * (l**3 - r**3)
    elif np.abs(diff) < 1.5:
        nonlinear = 0.003 * (l**4 - r**4)
    else:
        nonlinear = 0.0025 * l * r * (l - r)
    
    # High-frequency chaotic term for discrete scale structure (P3)
    periodic = 0.004 * np.sin(25.13 * (l + r)) + 0.0015 * np.cos(12.56 * diff)
    
    # Strong entropy-drive / variance injection (P4)
    entropy_drive = 0.003 * np.log1p(np.abs(l * r) + 1e-6) + 0.001 * np.abs(diff)
    
    # Sibling repulsion to keep branches diverse
    repulsion = 0.0025 * diff**2
    
    # Depth-dependent injection to fight collapse
    injection = 0.001 * np.random.normal(0, 0.2)
    
    val = l + r + accel + nonlinear + periodic + entropy_drive + repulsion + injection
    return safe_tanh(val) * 0.62   # very low damping to keep system alive
...
print("Paste the full output back. This v9 is the boldest redesign yet.")