...
def ff_fold_v8(l, r):
    ...
    nonlinear = 0.005 * (l**3 - r**3) + 0.002 * (l**4 - r**4) + 0.001 * l * r * (l - r)
    
    # High-frequency periodic + chaotic term for discrete scale structure (P3)
    periodic = 0.004 * np.sin(25.13 * (l + r)) + 0.001 * np.cos(12.56 * (l - r))
    
    # Explicit entropy-drive / variance injection (P4)
    entropy_drive = 0.0025 * np.log1p(np.abs(l * r) + 1e-6)
    
    # Strong sibling repulsion to keep branches diverse
    repulsion = 0.002 * (l - r)**2 + 0.0008 * np.abs(l - r)
    
    # Small depth-dependent injection to fight collapse
    injection = 0.0008 * np.random.normal(0, 0.15)
    
    val = l + r + accel + nonlinear + periodic + entropy_drive + repulsion + injection
    return safe_tanh(val) * 0.65   # significantly lower damping
...
print("=== Finsler-Friedmann 2025 — TOE Checklist v8 (Depth 24) ===\n")
...
print("\n=== Finsler-Friedmann TOE Checklist v8 complete ===")
print("Paste the full output back. This v8 is the most aggressive redesign yet.")