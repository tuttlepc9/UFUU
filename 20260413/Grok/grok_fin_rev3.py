...
def ff_fold_v3(l, r):
    # Core Finsler acceleration (kept from original success)
    accel = 0.0025 * (l**2 + r**2) * (l - r)
    
    # New terms to create multiple attractors + entropy flow
    nonlinear = 0.001 * (l**3 - r**3)          # breaks symmetry, creates discrete scales
    periodic  = 0.0008 * np.sin(2 * np.pi * (l + r))   # discrete scale structure
    entropy_drive = 0.0003 * np.log1p(np.abs(l * r) + 1e-8)   # prevents entropy collapse
    
    val = l + r + accel + nonlinear + periodic + entropy_drive
...
print("=== Finsler-Friedmann 2025 — TOE Checklist v3 (Depth 24) ===\n")
...
print("\n=== Finsler-Friedmann TOE Checklist v3 complete ===")
print("Paste the full output and we will immediately refine further if needed.")